...
(function(){
  ...
  function render(mode){
    ...
    selectedMode=mode; desc.textContent=c.description||'';
    panel.innerHTML=`<div class="mode-name">${c.label}</div>`+(c.fields||[]).map(([l,v])=>`<div class="field"><div class="label">${l}</div><div class="control"><button type="button">‹</button><div class="value">${v}</div><button type="button">›</button></div></div>`).join('')+`<div class="dots">${[0,1,2,3].map(i=>`<span class="dot ${i===c.dot?'on':''}"></span>`).join('')}</div>`;
  ...
  document.getElementById('play').addEventListener('click',()=>send('play',{mode:selectedMode,config:cfg[selectedMode]}));
  ...
  addEventListener('resize',resize);resize();requestAnimationFrame(draw);render(selectedMode);send('ready',{});
})();
