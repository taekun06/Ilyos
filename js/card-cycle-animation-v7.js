...
(() => {
  ...
  if (window.__ILYOS_CARD_CYCLE_V7__) return;
  window.__ILYOS_CARD_CYCLE_V7__ = true;
  ...
  function isLocalVisualTurn({ strict = false } = {}) {
    ...
    if (strict) {
      const legacy = ['hudV2MoveCount', 'hudV2PushCount', 'hudV2MagicCount'].map(byId).filter(Boolean);
      if (miniCards().length && legacy.length === 3 && legacy.every(btn => btn.disabled)) return false;
    }
    return true;
  }
  ...
  async function runDeal(cards) {
    if (!isLocalVisualTurn({ strict: true })) return;
  ...
  function start() {
    ...
    inspectDeck();
    window.ILYOS_CARD_CYCLE_V7 = { inspect: inspectDeck, stop: () => observer?.disconnect() };
  }
